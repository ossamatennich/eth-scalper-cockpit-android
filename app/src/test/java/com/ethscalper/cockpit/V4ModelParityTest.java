...
public class V4ModelParityTest {
    ...
    @Test public void decisionMaterialMatchesFrozenHashes()throws Exception{JSONObject m=new JSONObject(new String(Files.readAllBytes(Path.of("src/main/assets/v4_frozen_hash_manifest.json")),java.nio.charset.StandardCharsets.UTF_8));assertFalse(m.getBoolean("realTradingAllowed"));JSONArray files=m.getJSONArray("files");Path root=Path.of("..");
        for(int i=0;i<files.length();i++){JSONObject f=files.getJSONObject(i);byte[] bytes=Files.readAllBytes(root.resolve(f.getString("path")));byte[] hash=MessageDigest.getInstance("SHA-256").digest(bytes);StringBuilder s=new StringBuilder();for(byte b:hash)s.append(String.format("%02x",b));assertEquals(f.getString("sha256"),s.toString());}}
}
