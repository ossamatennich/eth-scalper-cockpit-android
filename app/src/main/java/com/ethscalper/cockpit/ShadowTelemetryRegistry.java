...
public final class ShadowTelemetryRegistry {
    ...
    public synchronized RangeUpdate observeRange(String key,SignalDecision signal,
        ...
        return observeRange(key,"",signal,metrics,snapshot,now,adverse,marketFresh,btcFresh);
    }
    public synchronized RangeUpdate observeRange(String key,String candidateSignature,SignalDecision signal,
            NormalizedSignalMetrics.Result metrics,MarketSnapshot snapshot,long now,
            double adverse,boolean marketFresh,boolean btcFresh) {
        ...
        if(r==null){r=new RangeRecord(key,candidateSignature,signal,now);ranges.put(key,r);rangeOrder.addLast(key);
        ...
        r.adverse=Math.max(r.adverse,Math.max(0,adverse));
        double reclaim=metrics.a>0&&finitePositive(snapshot.marketLast)&&finitePositive(signal.movementExtreme)
                ?("LONG".equals(signal.side)?snapshot.marketLast-signal.movementExtreme
                :signal.movementExtreme-snapshot.marketLast)/metrics.a:Double.NaN;
        if(Double.isFinite(reclaim)){r.reclaimInA=reclaim;changed|=r.reclaim(reclaim,r.adverse,now);}
        r.distanceFromOriginInA=metrics.a>0&&finitePositive(snapshot.marketLast)&&finitePositive(signal.movementOrigin)
    ...
    public synchronized BreakoutUpdate observeBreakout(String key,SignalDecision signal,
        ...
        return observeBreakout(key,"",signal,snapshot,createdAt,targetAt,adverse,favorable,reason,firstExecutableAt);
    }
    public synchronized BreakoutUpdate observeBreakout(String key,String candidateSignature,SignalDecision signal,
            MarketSnapshot snapshot,long createdAt,long targetAt,double adverse,double favorable,
            String reason,long firstExecutableAt) {
        ...
        r=new BreakoutRecord(key,candidateSignature,signal,createdAt,targetAt,adverse,favorable,reason,firstExecutableAt,snapshot);
    ...
    public synchronized List<Map<String,Object>> rangeSnapshots(){List<Map<String,Object>> out=new ArrayList<>();
        for(String key:rangeOrder){RangeRecord r=ranges.get(key);if(r!=null)out.add(r.snapshot());}return out;}
    public synchronized List<Map<String,Object>> noRetraceSnapshots(){List<Map<String,Object>> out=new ArrayList<>();
        for(String key:breakoutOrder){BreakoutRecord r=breakouts.get(key);if(r!=null)out.add(r.snapshot());}return out;}
    ...
    private static final class RangeRecord{
        ...
        double adverse,maxAdverse025,maxAdverse050,maxAdverse075,reclaimInA=Double.NaN,
                distanceFromOriginInA=Double.NaN;boolean feedFresh,btcFresh;
        NormalizedSignalMetrics.Result metrics;
        RangeRecord(String key,String signature,SignalDecision s,long now){this.key=key;this.signature=signature==null?"":signature;side=s.side;family=s.family;
        ...
        Map<String,Object> details(){LinkedHashMap<String,Object>d=new LinkedHashMap<>();d.put("movementKey",key);
            d.put("candidateSignature",signature);
            d.put("originalEntry",originalEntry);d.put("movementOrigin",origin);d.put("movementExtreme",extreme);
            d.put("reclaimReference","MOVEMENT_EXTREME");d.put("reclaimReferencePrice",number(extreme));
            d.put("reclaimInA",number(reclaimInA));d.put("distanceFromMovementOriginInA",number(distanceFromOriginInA));
        ...
        Map<String,Object> snapshot(){LinkedHashMap<String,Object>d=new LinkedHashMap<>();d.put("movementKey",key);
            d.put("candidateSignature",signature);d.put("movementExtreme",number(extreme));d.put("reclaimReference","MOVEMENT_EXTREME");
            d.put("firstPriceReclaim025AAt",nullable(firstPriceReclaim025AAt));d.put("firstPriceReclaim050AAt",nullable(firstPriceReclaim050AAt));
            d.put("firstPriceReclaim075AAt",nullable(firstPriceReclaim075AAt));d.put("targetTouchedAt",nullable(targetTouchedAt));
            d.put("stopTouchedAt",nullable(stopTouchedAt));return d;}
    }
    ...
    private static final class BreakoutRecord{
        ...
        BreakoutRecord(String key,String signature,SignalDecision s,long created,long targetAt,double adverse,double favorable,
                       String reason,long executable,MarketSnapshot x){this.key=key;symbol=s.symbol;side=s.side;family=s.family;
        ...
        Map<String,Object> snapshot(){LinkedHashMap<String,Object>d=new LinkedHashMap<>();d.put("movementKey",key);
            d.put("candidateSignature",signature);d.put("createdAt",createdAt);d.put("targetTouchedAt",targetAt);
            d.put("duplicatesGrouped",duplicates);d.put("side",side);d.put("family",family);d.put("reasonCode",reason);return d;}
    }
