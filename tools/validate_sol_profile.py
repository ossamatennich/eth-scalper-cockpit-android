...
def validate_profile(sol_rows):
    ...
    for _,_,_,_,entry,_ in sol_rows:
        ...
        if slmin>slcap or max(tpfloor,1.95*cost)>tpcap:raise RuntimeError(f"Profile distance inversion at SOL={entry}")
        for budget in (10,11.14,12.28,13.41,14.55):
            risk=slmin+allow;q=math.floor(budget/risk)
            if q<1 or q>120 and q*0<0:raise RuntimeError("Invalid quantity")
            q=min(q,120)
            if q*risk>budget+1e-9:raise RuntimeError("Budget exceeded")
        checked+=1
    return checked
def main():
    parser=argparse.ArgumentParser();parser.add_argument("--cache",default="build/sol-profile-cache");parser.add_argument("--report",default="SOL_PROFILE_V1_RESEARCH_REPORT.md");parser.add_argument("--manifest",default="build/sol-profile-corpus-manifest.json");parser.add_argument("--end-date",default=(dt.date.today()-dt.timedelta(days=1)).isoformat());args=parser.parse_args()
    ...
    ratios=[];eth_ret=[];sol_ret=[];monthly={};manifest=[];counts={s:0 for s in SYMBOLS};gaps={s:0 for s in SYMBOLS};dups={s:0 for s in SYMBOLS};first=None;last=None;profile_checked=0
    for period,daily in periods:
        ...
        key=period[:7];monthly.setdefault(key,[]).extend(local);profile_checked+=validate_profile(data["SOLUSDT"])
    ...
    manifest_doc={"generatedAt":dt.datetime.now(dt.timezone.utc).isoformat(),"officialSource":BASE,"periodStart":"2024-07-01","periodEnd":end.isoformat(),"archives":manifest,"counts":counts,"gaps":gaps,"duplicates":dups}
    ...
    Path(args.report).write_text(report,encoding="utf-8");print(json.dumps({"manifestSha256":manifest_doc["manifestSha256"],"counts":counts,"ratios":stats,"observations":len(ratios),"correlation":corr(eth_ret,sol_ret)},indent=2))
