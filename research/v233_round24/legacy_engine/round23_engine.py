...
def setup_events(f: pd.DataFrame, s: Spec) -> list[dict]:
    ...
    if s.architecture=='TREND_PULLBACK_RESUMPTION':
        ...
        upper=f.bb_upper_22.to_numpy(float); lower=f.bb_lower_22.to_numpy(float); mid=f.bb_mid.to_numpy(float)
...
def run_backtest(f:pd.DataFrame,s:Spec,start:pd.Timestamp=START,end:pd.Timestamp=END,fee:float=.0004,slip:float=.0001,entry_delay_bars:int=0,extra_entry_penalty:float=0.0):
    ...
    def close(p,i,reason,px):
        ...
        rows.append({'architecture':s.architecture,'side':s.side,'signal_time':idx[p['signal_i']],'entry_time':idx[p['entry_i']],'exit_time':idx[i],
                     ...
                     'bars_held':i-p['entry_i']+1,'net_pnl':net,'fees':fees,'r_net':net/risk_cash if risk_cash else 0.,'equity_after':equity,
    ...
    def exit_check(p,i,allow_time=True):
        held=i-p['entry_i']+1
        if allow_time and held>=s.max_hold_bars: return 'MAX_HOLD',adverse(O[i],s.side,'EXIT',slip)
...
def nested_monthly(variants,purge_h=48,embargo_h=24):
    ...
    for st in pd.date_range(pd.Timestamp('2022-01-01',tz='UTC'),END-pd.offsets.MonthBegin(1),freq='MS'):
        ...
        outer=slice_entries(all_t,test_st,test_en) if test_st<test_en else all_t.iloc[0:0]
        if len(outer):
...
def expanding_annual(variants,purge_h=48,embargo_h=24):
    ...
    for y in (2022,2023,2024,2025,2026):
        ...
        for s,t,mtab in cache:
            train=t[(t.entry_time<train_end)&(t.exit_time<train_end)].copy();scored.append((select_score_cached(train,mtab,train_end),json.dumps(asdict(s),sort_keys=True),s,t,metrics(train)))
        scored.sort(key=lambda z:(z[0],z[1]),reverse=True);score,_,chosen,all_t,tm=scored[0];outer=slice_entries(all_t,test_st,test_en)
...
def bootstrap_month(t,n=10000):
    if len(t)==0:return {'samples':n,'p_mean_positive':0.,'q10_mean_r':None}
    z=t.copy();z['month']=z.entry_time.dt.to_period('M').astype(str);g=z.groupby('month').r_net.agg(['sum','count']);sums=g['sum'].to_numpy(float);cnt=g['count'].to_numpy(float)
    rng=np.random.default_rng(SEED+1);pick=rng.integers(0,len(sums),size=(n,len(sums)));means=sums[pick].sum(1)/np.maximum(cnt[pick].sum(1),1)
    return {'samples':n,'p_mean_positive':float((means>0).mean()),'q10_mean_r':float(np.quantile(means,.1))}


def monte_carlo(r,n=10000,risk=.0015):
    if len(r)==0:return {'samples':n,'q95_max_drawdown_abs':None}
    rng=np.random.default_rng(SEED+2);dds=np.empty(n);batch=500
    for j in range(0,n,batch):
        k=min(batch,n-j);order=np.argsort(rng.random((k,len(r))),axis=1);rr=r[order];eq=np.cumprod(1+risk*rr,axis=1);peak=np.maximum.accumulate(np.c_[np.ones(k),eq],axis=1)[:,1:];dds[j:j+k]=np.abs(np.min(eq/peak-1,axis=1))
    return {'samples':n,'q50_max_drawdown_abs':float(np.quantile(dds,.5)),'q95_max_drawdown_abs':float(np.quantile(dds,.95)),'q99_max_drawdown_abs':float(np.quantile(dds,.99))}
...
def main():
    ...
    for central in central_specs():
        ...
        rr=ct.r_net.to_numpy(float) if len(ct) else np.array([]);iid=bootstrap_iid(rr);blocks=bootstrap_month(ct);mc=monte_carlo(rr)
