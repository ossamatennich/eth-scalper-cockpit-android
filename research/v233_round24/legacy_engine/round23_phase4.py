...
def setup_events(f,s):
 ...
 if s.architecture in ('BREAKOUT_FOLLOW_THROUGH_CONFIRMATION','BREAKOUT_RETEST_CONTINUATION'):
  ...
  base=common&dir_ok&(adx>=s.h4_adx_min)&br&(f.range_atr.to_numpy(float)>=s.range_atr_min)&body&(clv>=.65)&(rv>=s.rel_volume_min);base[:120]=False
...
def run_backtest(f,s,start=e.START,end=e.END,fee=.0004,slip=.0001,entry_delay_bars=0,extra_entry_penalty=0.):
 ...
 for i in range(max(1,si),min(ei,len(f))):
  if pos is not None:
   ...
   if held>=s.max_hold_bars:close(pos,i,'MAX_HOLD',e.adverse(O[i],s.side,'EXIT',slip));pos=None;continue
...
def main():
 ...
 for central in central_specs():
  ...
  rr=ct.r_net.to_numpy(float) if len(ct) else np.array([]);iid=e.bootstrap_iid(rr);blocks=e.bootstrap_month(ct);mc=e.monte_carlo(rr);gate=e.gate_report(ct,month,quarter,nt,at,af,bt,bf,stresses,neigh,iid,blocks,mc,reg,yrs,criteria)
