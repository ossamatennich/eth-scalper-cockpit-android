...
def neighbours(s):
 ...
 elif s.architecture=='SLEEVE_CONSECUTIVE_LOSS_COOLDOWN':add('consecutive_losses',[2,3]);add('pause_hours',[24,72])
 else:add('portfolio_rolling_closed_trades',[3,7]);add('portfolio_loss_sum_trigger_r',[-1.,-2.]);add('portfolio_pause_hours',[24,72]);add('sleeve_consecutive_losses',[2,3]);add('sleeve_pause_hours',[24,72])
...
def main():
 ...
 ah=verify(a.protocol/'ROUND23_PHASE7_ARCHITECTURES_LOCKED.json');ch=e.verify_lock(a.protocol/'ROUND23_CRITERIA_LOCKED.json');crit=json.loads((a.protocol/'ROUND23_CRITERIA_LOCKED.json').read_text());f=e.prepare_features(e.load_ohlcv(a.corpus));reports=[]
 for c in central_specs():
  ...
  for r in crit['stress_matrix']:stress[r['id']]=e.metrics(governed(f,c,r['fee'],r['slippage'],r['entry_delay_bars'],r['extra_entry_penalty']))
 ...
 d={'phase':'ROUND23_PHASE7','stable':any(x['passes_all'] for x in reports),'admitted_count':sum(x['passes_all'] for x in reports),'round22_holdout_used':False,'scalp_engine_touched':False,'android_integration_allowed':False,'protocol_hashes':{'phase7_architectures':ah,'criteria':ch},'candidate_reports':reports};(a.output/'PHASE7_DECISION.json').write_text(json.dumps(e.json_safe(d),indent=2));print(json.dumps(e.json_safe(d),indent=2))
