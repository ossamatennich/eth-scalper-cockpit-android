...
def run_backtest(f:pd.DataFrame,s:P3Spec,start=e.START,end=e.END,fee=.0004,slip=.0001,entry_delay_bars=0,extra_entry_penalty=0.):
    ...
    for i in range(max(1,si),min(ei,len(f))):
        if pos is not None:
            ...
            if held>=s.time_decay_bars and pos['max_fav_r']<s.minimum_favourable_r:
...
def main():
    ...
    for central in central_specs():
      ...
      rr=ct.r_net.to_numpy(float) if len(ct) else np.array([]);iid=e.bootstrap_iid(rr);blocks=e.bootstrap_month(ct);mc=e.monte_carlo(rr);gate=e.gate_report(ct,month,quarter,nt,at,af,bt,bf,stresses,neigh,iid,blocks,mc,regimes,years,criteria)
