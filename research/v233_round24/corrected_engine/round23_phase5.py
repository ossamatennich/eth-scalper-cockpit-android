...
def main():
 ...
 for c in specs():
  ...
  rr=ct.r_net.to_numpy(float) if len(ct) else np.array([]);iid=e.bootstrap_iid(rr);bl=e.bootstrap_month(ct);mc=e.monte_carlo(ct);gate=e.gate_report(ct,mo,qu,nt,at,af,bt,bf,stress,nr,iid,bl,mc,rg,yr,crit)
