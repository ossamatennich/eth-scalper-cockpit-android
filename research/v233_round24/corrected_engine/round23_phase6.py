...
def main():
 ...
 for c in specs():
  ...
  reports.append({'architecture':c.architecture,'side':'LONG_SHORT_COMBINED_LOCKED','passes_all':passes_all,'central':e.metrics(ct),'direction_metrics':side_metrics(ct),'purged_monthly_outer':e.metrics(nt),'expanding_annual_outer':e.metrics(at),'blocked_year_outer':e.metrics(bt),'failed_checks':{k:v['diagnostics']['failed_checks'] for k,v in gates.items()},'report_path':str(Path(c.architecture)/'REPORT.json'),'minimum_annual_expectancy':float(af[af.outer_trades>0].outer_expectancy_r.min()) if len(af) and (af.outer_trades>0).any() else -math.inf,'mc_q95_dd':mc['q95_max_drawdown_abs']})
