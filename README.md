...
La version courante est **2.34.4.3** (`versionCode 23443`). L’APK recommandée est l’édition **NMC Stable 4.3**, signée durablement afin que ses mises à jour Android remplacent réellement la version stable précédente. Elle mesure en shadow la qualité et la fréquence utile des confirmations ETH/SOL sans bloquer, publier ou modifier le moindre plan public. Le flux Binance Futures et le fallback REST public restent indépendants du volume des diagnostics. L’application ne passe aucun ordre : l’exécution reste entièrement manuelle.
...
- la politique `SHADOW_V23443_20260802` et le schéma `SHADOW_SCHEMA_V4` restent silencieux et purement observationnels.
